...
export async function updateTeamSettings(input: UpdateTeamSettingsInput): Promise<TeamPageData> { const name = input.name.trim().replace(/\s+/g, " "); if (name.length < 2 || name.length > 20) throw createError("INVALID_TEAM_NAME", "小队名称需为 2 至 20 个字符。" ); await requireAction("updateTeamSettings"); await call<{ updated: boolean }>("updateTeamSettings", { input: { name, anonymityMode: input.anonymityMode, allowMemberInvite: input.allowMemberInvite } }); return getMyTeam(); }
